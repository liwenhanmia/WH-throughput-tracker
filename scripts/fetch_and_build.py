...
def open_sheet(gc, sid, retries=4, wait=30):
    """Open a spreadsheet by ID with retry — handles empty-response 429 variants."""
...
# ─── Actual data (placeholder — fill in once tab layout confirmed) ────
def read_actual(sh):
    def open_tab(*names):
        for n in names:
    ...
    # ── TODO: fill in actual row/col positions once confirmed ──────────
...
def fetch_all():
    ...
    for idx, (wh, sid) in enumerate(cfg.items()):
        ...
        # Read each model sheet ONCE (3 API calls total per WH)
        bm = {
        ...
        print(f"  ✓ {wh}: bm_ib={bm.get('bm_ib',0):.0f}"
...
def wh_js(wh, d):
    ...
    mto = d.get('mto', {})
    ibs = d.get('ibSteps', {});  obs = d.get('obSteps', {})
    th  = d.get('theo', {})
    return (
...
def inject(all_bm, all_wh):
    ...
    for wh in ['PHB', 'PHL', 'PHIXC']:
        ...
        s = html.find(tag)
        if s == -1:
            print(f"  ⚠ BM_{wh} not found in HTML")
            continue
...
if __name__ == '__main__':
    print("=" * 55)
    print("WH Throughput Tracker — GSheet → HTML Builder v3")
    print("=" * 55)
